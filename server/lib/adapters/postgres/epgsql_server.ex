...
defmodule Realtime.Adapters.Postgres.EpgsqlServer do
  ...
  def init(
      ...
      when is_map(epgsql_params) and is_list(publications) and
             (is_binary(slot_name) or is_atom(slot_name)) and is_tuple(wal_position) do
  ...
  @impl true
  def handle_continue(:db_connect, %State{epgsql_params: epgsql_params} = state)
      when is_map(epgsql_params) do
    ...
    replication_epgsql_pid =
      case :epgsql.connect(epgsql_replication_config) do
        {:ok, replication_epgsql_pid} -> replication_epgsql_pid
        {:error, _error} -> nil
      end

    select_epgsql_pid =
      case :epgsql.connect(epgsql_select_config) do
        {:ok, select_epgsql_pid} -> select_epgsql_pid
        {:error, _error} -> nil
      end

    with true <- Enum.all?([replication_epgsql_pid, select_epgsql_pid], &is_pid(&1)),
         %State{} = updated_state <- %{
           state
      ...
      error ->
        is_pid(replication_epgsql_pid) && :epgsql.close(replication_epgsql_pid)
        is_pid(select_epgsql_pid) && :epgsql.close(select_epgsql_pid)
        {:stop, error, state}
    end
  end

  def handle_continue(:db_connect, state), do: {:stop, :bad_epgsql_params_format, state}
  ...
  def handle_call(
      ...
      when is_integer(xlog) and is_integer(offset) and is_pid(replication_epgsql_pid) do
  ...
  def handle_info(
      ...
      when is_pid(replication_epgsql_pid) and is_pid(select_epgsql_pid) do
  ...
  @impl true
  def handle_info(msg, state), do: {:stop, msg, state}
  ...
  defp start_replication(
       ...
       when is_binary(publication_name) and is_pid(replication_epgsql_pid) and
              is_pid(select_epgsql_pid) and is_binary(slot_name) and is_binary(xlog) and
              is_binary(offset) do
    case does_publication_exist(state) do
      true ->
        with :ok <- maybe_create_replication_slot(state),
             ...
             %State{} = updated_state <- reset_delays(state) do
          {:ok, updated_state}
        else
  ...
  defp start_replication(_state), do: {:error, :start_replication_bad_state}

  defp reset_delays(%State{delays: _} = state) do
    %{state | delays: [0]}
  end

  defp reset_delays(_state), do: :reset_delays_bad_state
  ...
  defp maybe_create_replication_slot(
       ...
       when is_pid(replication_epgsql_pid) and is_binary(slot_name) and
              is_binary(create_replication_command) do
  ...
  defp maybe_create_replication_slot(_state),
    do: {:error, :maybe_create_replication_slot_bad_state}

  defp maybe_drop_replication_slot(%State{
         select_epgsql_pid: select_epgsql_pid,
         slot_config: {slot_name, _command}
       })
       when is_pid(select_epgsql_pid) and is_binary(slot_name) do
  ...
  defp maybe_drop_replication_slot(_state), do: {:error, :maybe_drop_replication_slot_bad_state}

  defp does_publication_exist(%State{
         publication_name: publication_name,
         select_epgsql_pid: select_epgsql_pid
       })
       when is_binary(publication_name) and is_pid(select_epgsql_pid) do
    publication_query =
  ...
  defp does_publication_exist(_state), do: {:error, :does_publication_exist_bad_state}
  ...
  defp does_replication_slot_exist(_state), do: {:error, :does_replication_slot_exist_bad_state}
end
